...
def _display_grid(form_name=TABLE_NOTE, 
    ...
    if form_name == TABLE_FACULTY:
        ...
        st.subheader(f"{faculty_name} : {primary_key}")
        

        tab1, tab2, tab3 = st.tabs(["Work", "Team", "Note"])
        ...
        with tab1:
            ...
            with DBConn() as _conn:
                ...
                """
                df_work = pd.read_sql(sql_stmt, _conn)
                # important to assign a new grid_response
                grid_resp_work = _display_grid_df(df_work,
                            page_size=5, 
                            grid_height=200,
        ...
        with tab2:
            ...
            with DBConn() as _conn:
                ...
                """
                df_team = pd.read_sql(sql_stmt, _conn)

                grid_resp_team = _display_grid_df(df_team,
                            page_size=5, 
                            grid_height=200,
                            clickable_columns=["url"],                                             
                        )
        ...
        with tab3:
            ...
            with DBConn() as _conn:
                ...
                """
                df_note = pd.read_sql(sql_stmt, _conn)

                grid_resp_note = _display_grid_df(df_note,
                            page_size=5, 
                            grid_height=200,
                            clickable_columns=["url"],                                             
                        )
